...
if st.button("تكييف ورقة العمل بالذكاء الاصطناعي"):
  if subject and topic:
    with st.spinner("جاري تكييف المادة وتوليد الملفات التربوية..."):
      try:
        # محاكاة توليد المحتوى المكيّف بشكل آمن نصياً ورياضياً
        adapted_content = f"""
        تقرير تكييف ورقة العمل:
        - المبحث: {subject}
        - المهارة: {topic}
        - الفئة: {student_level}
        
        1. الهدف التعليمي المعدل: أن يتعرف الطالب على المفاهيم الأساسية بطريقة مبسطة.
        2. الاستراتيجيات والوسائل: استخدام الدعم البصري واللمسي، وتجزئة المهمة إلى خطوات صغيرة.
        3. الأسئلة والتمارين: تم تكييف الأسئلة لتناسب مستوى الأداء الفردي.
        """
        ...
        st.error(f"حدث خطأ أثناء الاتصال بالذكاء الاصطناعي أو معالجة الملفات: {e}")
  else:
