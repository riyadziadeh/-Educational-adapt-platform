...
def init_db():
    ...
    # =====================================================================================
    # =====================================================================================
    # === إصلاح خطأ حقيقي (اكتُشف بعد النشر الفعلي): على PostgreSQL، عندما يفشل
    # أمر ALTER TABLE (لأن العمود موجود أصلاً من تشغيل سابق)، تبقى المعاملة
    # الحالية (Transaction) على الاتصال في حالة "معطوبة" (Aborted) حتى يُنفَّذ
    # ROLLBACK صريح — وبما أن الاتصال مخزَّن مؤقتاً ويُعاد استخدامه (st.cache_resource)،
    # كانت هذه الحالة المعطوبة تنتقل لكل الاستعلامات اللاحقة على نفس الاتصال (بما
    # فيها إنشاء بقية الجداول أدناه)، فيفشل init_db() بالكامل ويظهر "تعذّر الاتصال
    # بقاعدة البيانات الدائمة" رغم أن الاتصال نفسه سليم تماماً. الحل: commit/rollback
    # مستقل لكل عمود على حدة، فيبقى الاتصال في حالة سليمة دائماً بغض النظر عن نجاح
    # أو فشل أي عمود بعينه. ===
    # =====================================================================================
    for alter_sql in [
        ...
        try:
            ...
            conn.rollback()  # العمود موجود أصلاً من تشغيل سابق — نُعيد الاتصال لحالة سليمة قبل المتابعة
