...
if __name__ == "__main__":
    ...
    if not api_key:
        ...
        # === إصلاح: أزلنا قيد type=["pdf","docx","txt"] عن عنصر الرفع نفسه. هذا القيد
        # كان يجعل ستريمليت يضبط خاصية accept بالمتصفح (اعتماداً على امتداد الملف فقط)،
        # وعلى بعض هواتف الأندرويد (خصوصاً لما يكون ملف الـ PDF قادماً من تطبيق سكانر أو
        # مشاركة واتساب/جوجل درايف بنوع MIME غير قياسي) يقوم "منتقي الملفات" بنظام
        # التشغيل بإخفاء/تعطيل هذا الملف من القائمة تماماً لأنه لا يطابق الفلتر المتوقّع —
        # فيظهر للمستخدم وكأن "التطبيق مش قابل يعمل أبلود لأي ورقة" رغم أن المشكلة أصلاً
        # قبل ما توصل لكودنا. الحل: نسمح باختيار أي ملف من الجهاز بلا فلترة على مستوى
        # المتصفح، ثم نتحقق من الامتداد بأنفسنا بعد الرفع (كما هو حاصل بالأسفل أصلاً)
        # ونعرض رسالة عربية واضحة لو كانت الصيغة غير مدعومة، بدل الاعتماد على فلترة
        # نظام التشغيل غير الموثوقة على كل الأجهزة. ===
        ...
        extracted_content = ""
        if uploaded_file is not None and uploaded_file.name.split(".")[-1].lower() not in SUPPORTED_EXTENSIONS:
            st.error(
                f"⚠️ صيغة الملف «{uploaded_file.name}» غير مدعومة. الرجاء رفع ملف PDF أو Word (.docx) أو نصي (.txt) فقط."
            )
            st.toast("❌ صيغة ملف غير مدعومة", icon="❌")
            uploaded_file = None
